...
UPDATE `formularios` SET `descripcion`='Escoger una carrera profesional va más allá de matricularse en un programa académico. En una gran proporción de casos, la carrera que se estudia es aquella que se desempeña a nivel profesional. No obstante, la elección no solo tiene que ver con las materias a estudiar o con las habilidades o personalidad propias. La elección también debe tener en cuenta tus preferencias sobre el tipo de ambiente de trabajo en el que te sientes cómodo; de esto depende también tu satisfacción personal y laboral.' WHERE `id`='33';
UPDATE `formularios` SET `descripcion`='A continuación veras un listado (imágenes de pirámides) de los 5 valores ocupacionales y los 5 intereses académicos mas importantes para ti seleccionados en las pruebas anteriores. A continuación te realizaremos preguntas de estos valores e intereses, respecto a los intereses de carrera seleccionados como nivel 3.' WHERE `id`='35';
